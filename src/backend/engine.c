...
GwOutputFunc gw_engine_get_append_edict_results_func ()
{
  return _output_append_edict_results;
}
GwOutputFunc gw_engine_get_append_kanjidict_results_func ()
{
  return _output_append_kanjidict_results;
}
GwOutputFunc gw_engine_get_append_examplesdict_results_func ()
{
  return _output_append_examplesdict_results;
}
GwOutputFunc gw_engine_get_append_unknowndict_results_func ()
{
  return _output_append_unknowndict_results;
}
GwOutputFunc gw_engine_get_append_less_relevant_header_func ()
{
  return _output_append_less_relevant_header;
}
GwOutputFunc gw_engine_get_append_more_relevant_header_func ()
{
  return _output_append_more_relevant_header;
}
GwOutputFunc gw_engine_get_pre_search_prep_func ()
{
  return _output_pre_search_prep;
}
GwOutputFunc gw_engine_get_after_search_cleanup_func ()
{
      ...
      if (gw_searchitem_existance_generic_comparison (item, GW_QUERYLINE_EXIST))
      {
        ...
        switch(relevance)
        {
          ...
          case GW_RESULT_MEDIUM_RELEVANCE:
              if ((item->dictionary->engine == GW_DICT_ENGINE_KANJI || item->total_irrelevant_results < MAX_MEDIUM_IRRELIVENT_RESULTS) &&
              ...
              break;
          default:
              if ((item->dictionary->engine == GW_DICT_ENGINE_KANJI || item->total_irrelevant_results < MAX_LOW_IRRELIVENT_RESULTS) &&
                    !item->show_only_exact_matches && 
                   (item->swap_resultline = gw_resultline_new ()) != NULL && item->target != GW_TARGET_KANJI)
              {
                //Store the result line and create an empty one in its place
