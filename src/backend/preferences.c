...
//!
void gw_pref_initialize ()
{
  char version[50];
  gw_pref_get_string_by_schema (version, GW_SCHEMA_BASE, GW_KEY_PROGRAM_VERSION, 50);

  if (strcmp(version, VERSION) != 0)
  {
    gw_pref_set_string_by_schema (GW_SCHEMA_BASE, GW_KEY_PROGRAM_VERSION, VERSION);
    gw_pref_reset_value_by_schema (GW_SCHEMA_DICTIONARY, GW_KEY_ENGLISH_SOURCE);
    gw_pref_reset_value_by_schema (GW_SCHEMA_DICTIONARY, GW_KEY_KANJI_SOURCE);
    gw_pref_reset_value_by_schema (GW_SCHEMA_DICTIONARY, GW_KEY_NAMES_PLACES_SOURCE);
    gw_pref_reset_value_by_schema (GW_SCHEMA_DICTIONARY, GW_KEY_EXAMPLES_SOURCE);
  }
}
...
//!
GSettings* gw_pref_get_settings_object (const char *schema)
{
    //Declarations
    GList *iter;
...
//!
void gw_pref_reset_value (GSettings* settings, const char *key)
{
...
//! @param key A string identifying the key to reset
//!
void gw_pref_reset_value_by_schema (const char* schema, const char *key)
{
    GSettings* settings;
    
    settings = gw_pref_get_settings_object (schema);
    if (settings != NULL)
    {
      gw_pref_reset_value (settings, key);
    }
}
...
//!
int gw_pref_get_int (GSettings *settings, const char *key)
{
...
//! @param backup the value to return on failure
//!
int gw_pref_get_int_by_schema (const char* schema, const char *key)
{
    GSettings* settings;
    int value;
    ...
    if (settings != NULL)
    {
      value = gw_pref_get_int (settings, key);
    }
...
//!
void gw_pref_set_int (GSettings *settings, const char *key, const int request)
{
...
//! @param request The value to set
//!
void gw_pref_set_int_by_schema (const char* schema, const char *key, const int request)
{
    GSettings* settings;
    ...
    if (settings != NULL)
    {
      gw_pref_set_int (settings, key, request);
    }
}
...
//! @param key The key to use to look up the pref
//!
gboolean gw_pref_get_boolean (GSettings *settings, const char *key)
{
...
//! @param key The key to use to look up the pref
//!
gboolean gw_pref_get_boolean_by_schema (const char* schema, const char *key)
{
    GSettings* settings;
    gboolean value;
    ...
    if (settings != NULL)
    {
      value = gw_pref_get_boolean (settings, key);
    }
...
//! @param request The value to set
//!
void gw_pref_set_boolean (GSettings *settings, const char *key, const gboolean request)
{
...
//! @param request The value to set
//!
void gw_pref_set_boolean_by_schema (const char* schema, const char *key, const gboolean request)
{
    GSettings* settings;
    ...
    if (settings != NULL)
    {
      gw_pref_set_boolean (settings, key, request);
    }
}
...
//!
void gw_pref_get_string (char *output, GSettings *settings, const char *key, const int n)
{
    gchar *value = NULL; 
...
//! @param n The max characters to copy to output
//!
void gw_pref_get_string_by_schema (char *output, const char *schema, const char *key, const int n)
{
    GSettings* settings;
    
    settings = gw_pref_get_settings_object (schema);

    gw_pref_get_string (output, settings, key, n);
}
...
//! @param request The value to set
//!
void gw_pref_set_string (GSettings *settings, const char *key, const char* request)
{
...
//! @param request The value to set
//!
void gw_pref_set_string_by_schema (const char* schema, const char *key, const char* request)
{
    GSettings *settings;
    ...
    if (settings != NULL)
    {
      gw_pref_set_string (settings, key, request);
    }
}
...
//!
gulong gw_pref_add_change_listener (GSettings *settings, const char *key, void (*callback_function) (GSettings*, gchar*, gpointer), gpointer data)
{
    g_assert (key != NULL);
...
//! @returns A gulong used to remove a signal later if desired
//!
gulong gw_pref_add_change_listener_by_schema (const char* schema, const char *key, void (*callback_function) (GSettings*, gchar*, gpointer), gpointer data)
{
    ...
    settings = gw_pref_get_settings_object (schema);
    id = gw_pref_add_change_listener (settings, key, callback_function, data);
...
//!
void gw_pref_remove_change_listener (GSettings *settings, gulong id)
{
...
//! @param id The signalid returned by gw_pref_add_change_listener
//!
void gw_pref_remove_change_listener_by_schema (const char* schema, gulong id)
{
    GSettings *settings;
    ...
    if (settings != NULL)
    {
      gw_pref_remove_change_listener (settings, id);
    }
}
