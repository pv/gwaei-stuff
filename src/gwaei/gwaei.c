...
GwApplication* gw_app_new (int* argc, char** argv[])
{
    ...
    if (temp != NULL)
    {
      ...
      temp->preferences = lw_preferences_new ();
      temp->dictinfolist = gw_dictinfolist_new (20, temp->preferences);
      temp->block_new_searches = 0;

      temp->tagtable = gw_texttagtable_new ();
      lw_preferences_add_change_listener_by_schema (temp->preferences, LW_SCHEMA_HIGHLIGHT, LW_KEY_MATCH_FG, gw_app_sync_tag_cb, temp);
      lw_preferences_add_change_listener_by_schema (temp->preferences, LW_SCHEMA_HIGHLIGHT, LW_KEY_MATCH_BG, gw_app_sync_tag_cb, temp);
      lw_preferences_add_change_listener_by_schema (temp->preferences, LW_SCHEMA_HIGHLIGHT, LW_KEY_HEADER_FG, gw_app_sync_tag_cb, temp);
      lw_preferences_add_change_listener_by_schema (temp->preferences, LW_SCHEMA_HIGHLIGHT, LW_KEY_HEADER_BG, gw_app_sync_tag_cb, temp);
      lw_preferences_add_change_listener_by_schema (temp->preferences, LW_SCHEMA_HIGHLIGHT, LW_KEY_COMMENT_FG, gw_app_sync_tag_cb, temp);
...
void gw_app_free (GwApplication *app)
{
    ...
    lw_preferences_free (app->preferences);
...
void gw_app_sync_tag_cb (GSettings *settings, gchar *key, gpointer data)
{
    ...
    if (gdk_rgba_parse (&color, hex) == FALSE)
    {
      ...
      lw_preferences_reset_value_by_schema (app->preferences, LW_SCHEMA_HIGHLIGHT, key);
      return;
    }
