...
G_MODULE_EXPORT void gw_dictionaryinstallwindow_source_entry_changed_cb (GtkWidget *widget, gpointer data)
{
    ...
    {
      lw_preferences_set_string_by_schema (app->preferences, window->di->schema, window->di->key, value);
    }
...
G_MODULE_EXPORT void gw_dictionaryinstallwindow_reset_default_uri_cb (GtkWidget *widget, gpointer data)
{
    ...
    lw_preferences_reset_value_by_schema (app->preferences, window->di->schema, window->di->key);
    lw_preferences_get_string_by_schema (app->preferences, value, window->di->schema, window->di->key, 200);
